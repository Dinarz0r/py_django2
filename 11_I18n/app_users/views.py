    ...
    if request.method == 'POST':
        ...
        if auth_form.is_valid():
            ...
            if user:
                if user.is_active:
                    login(request, user)
                    return HttpResponse(_('You are successfully logged in'))
                else:
                    auth_form.add_error('__all__', _('Error! The user account is inactive.'))
            else:
                auth_form.add_error('__all__', _('Error! Check the spelling of your username and password.'))
    else:
        auth_form = AuthForm()
...
def logout_view(request):
    logout(request)
    return HttpResponse(_('You have successfully logged out of your account'))
        ...
        if form.is_valid():
            ...
            if current_user:
                ...
                send_mail(subject=_('Password recovery'),
                          message=_("New password:") + ' ' + new_password,
                          from_email='admin@company.com',
                          recipient_list=[form.cleaned_data['email']])
                return HttpResponse(_('An email with a new password was successfully sent'))
